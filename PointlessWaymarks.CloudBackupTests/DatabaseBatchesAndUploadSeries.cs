...
{
    public DirectoryInfo DbDirectory { get; set; } = null!;
    public IS3AccountInformation S3Credentials { get; set; } = null!;
    public DirectoryInfo TestDirectory { get; set; } = null!;
    public DirectoryInfo TestDirectory1 { get; set; } = null!;
    public DirectoryInfo TestDirectory2 { get; set; } = null!;
    public DirectoryInfo TestDirectory3 { get; set; } = null!;
    public FileInfo TestFile1 { get; set; } = null!;
    public FileInfo TestFile2 { get; set; } = null!;
    public FileInfo TestFile3 { get; set; } = null!;
    public FileInfo TestFile3Duplicate { get; set; } = null!;
    public FileInfo TestFile4 { get; set; } = null!;
    public FileInfo TestFile5 { get; set; } = null!;
    public FileInfo TestFile6 { get; set; } = null!;
    public FileInfo TestFile7 { get; set; } = null!;
    ...
    public async Task T000_InitialTransferAndFileCheck()
    {
        ...
        var batch1 = testBatch;

        Assert.Multiple(() =>
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(8));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(0));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(8));
        });
    ...
    public async Task T010_SecondTransferWithNewFileCheck()
    {
        ...
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(1));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(0));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(8));
        });
    ...
    public async Task T020_ThirdTransferWithDeletedFile()
    {
        ...
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(1));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(7));
        });
    ...
    public async Task T100_ThirdTransferWithAddedDirectoryExclusion()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(4));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(3));
        });
    ...
    public async Task T200_FourthTransferWithAddsFromRemovingAddedDirectoryExclusionAndAdditionalDelete()
    {
        ...
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(4));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(1));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(6));
        });
    ...
    public async Task T300_FifthTransferDirectoryRenameTriggersFileCopy()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(4));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(4));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(6));
        });
    ...
    public async Task T400_FifthTransferDuplicateDirectory()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(0));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(10));
        });
    ...
    public async Task T500_FifthTransferDuplicateDirectoryAndDeletePrevious()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(batch1.CloudCopies, Has.Count.EqualTo(4));
            Assert.That(batch1.CloudUploads, Has.Count.EqualTo(0));
            Assert.That(batch1.CloudDeletions, Has.Count.EqualTo(8));
            Assert.That(batch1.FileSystemFiles, Has.Count.EqualTo(6));
        });

...
public class MagicScalerImageProcessor : IPictureResizer
{
    ...
    public static async Task<FileInfo?> ConvertToJpeg(FileInfo toConvert)
    {
        if (!toConvert.Exists) return null;

        var directory = toConvert.Directory;

        if (directory is null) return null;

        var newFileInfo =
            UniqueFileTools.UniqueFile(directory, $"{Path.GetFileNameWithoutExtension(toConvert.Name)}.jpg");
